class KeyBoard extends egret.EventDispatcher {
	...
	public static shieldingHotKey: string[] = [];
	...
	//检测屏蔽热键
	private checkShieldingHotKey(e): boolean {
		let key = "";
		for (var item in this.keyValue) {
			if (parseInt(item) == e.keyCode) {
		...
		for (let i = 0; i < KeyBoard.shieldingHotKey.length; i++) {
	 ...
	 * 设置屏蔽浏览器的热键
	 */
	public static setShieldingHotKey(tKeyList: string[]): void {
